...
import { CheckCircle, Clock, Edit, ImageIcon, Loader2, MoreVertical, Sparkles, Trash2 } from "lucide-react";
...
export function MyListingsClient({ listings }: { listings: Listing[] }) {
	...
	async function handleMarkAsSold(id: number) {
		...
		<>
			<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
				{listings.map((listing) => {
					...
					const daysLeft = isBoosted
						? Math.ceil((new Date(listing.boostedUntil!).getTime() - Date.now()) / (1000 * 60 * 60 * 24))
						: 0;
					...
					return (
						<div
							key={listing.id}
							className={`group relative overflow-hidden rounded-xl border bg-white transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-black/5 ${
								isBoosted ? "border-[#F59E0B]/40 ring-1 ring-[#F59E0B]/20" : "border-[#E2E8F0] hover:border-[#93C5FD]"
							}`}
						>
							{/* Image */}
							<Link href={`/listing/${listing.id}`} className="relative block aspect-[16/10] overflow-hidden bg-[#F1F5F9]">
								{imageUrl ? (
									<Image
										src={imageUrl}
										alt={listing.title}
								...
								{listing.status === "sold" && (
									<div className="absolute inset-0 flex items-center justify-center bg-black/50">
										<span className="rounded-full bg-white px-3 py-1 text-xs font-bold uppercase text-[#0F172A]">Sold</span>
									</div>
								)}

								{/* Top badges */}
								<div className="absolute left-2 top-2 flex gap-1.5">
									<span className={`rounded-md px-2 py-0.5 text-[10px] font-bold uppercase shadow-sm ${
									...
									{isBoosted && (
										<span className="flex items-center gap-0.5 rounded-md bg-[#F59E0B] px-2 py-0.5 text-[10px] font-bold text-white shadow-sm">
							...
							<div className="p-3.5">
								<Link href={`/listing/${listing.id}`}>
									...
									<h3 className="mt-0.5 truncate text-sm text-[#334155]">{listing.title}</h3>
								...
								<div className="mt-3 flex items-center gap-2 border-t border-[#F1F5F9] pt-3">
									<Link href={`/listing/${listing.id}/edit`} className="flex-1">
										<button className="flex h-8 w-full items-center justify-center gap-1.5 rounded-lg border border-[#E2E8F0] bg-white text-xs font-medium text-[#475569] transition-all hover:border-[#93C5FD] hover:text-[#1E40AF] active:scale-[0.97]">
									...
									{!isBoosted && listing.status === "published" && (
										<button
											className="flex h-8 flex-1 items-center justify-center gap-1.5 rounded-lg bg-gradient-to-r from-[#F59E0B] to-[#FBBF24] text-xs font-bold text-[#0F172A] shadow-sm transition-all hover:shadow-md hover:shadow-amber-500/15 active:scale-[0.97]"
											onClick={() => {
									...
									<DropdownMenu>
										...
										<DropdownMenuContent align="end" className="w-44">
											{listing.status === "published" && (
												<DropdownMenuItem onClick={() => handleMarkAsSold(listing.id)} className="text-sm">
			...
			<Dialog open={showBoostDialog} onOpenChange={setShowBoostDialog}>
				<DialogContent className="sm:max-w-md">
					<DialogHeader>
						<div className="mx-auto mb-2 flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-[#F59E0B] to-[#FBBF24] shadow-lg shadow-amber-500/20">
							<Sparkles className="h-6 w-6 text-white" />
						</div>
						<DialogTitle className="text-center text-xl">Boost your listing</DialogTitle>
					...
					<div className="space-y-2.5 py-3">
						{(["7", "14", "30"] as BoostDuration[]).map((duration) => {
							...
							return (
								...
								>
									{popular && (
										<span className="absolute -top-2.5 right-3 rounded-full bg-[#F59E0B] px-2 py-0.5 text-[10px] font-bold text-white">
											Popular
										</span>
									)}
									<div className="flex items-center gap-3">
										...
										<div>
											...
											<p className="text-xs text-[#64748B]">{duration} days of visibility</p>
										</div>
									</div>
									<p className="text-base font-bold text-[#0F172A]">{boostPrices[duration]} <span className="text-xs font-medium text-[#64748B]">XAF</span></p>
								</label>
							);
						})}
					</div>
					<DialogFooter className="flex-col gap-2 sm:flex-col">
						<Button
							className="w-full rounded-xl bg-gradient-to-r from-[#F59E0B] to-[#FBBF24] font-bold text-[#0F172A] shadow-md shadow-amber-500/20 hover:shadow-lg"
							onClick={handleBoost}
							disabled={isBoosting}
						>
							{isBoosting ? (
