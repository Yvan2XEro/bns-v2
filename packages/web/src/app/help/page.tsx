			...
			<div className="container mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
					...
					{categories.map((cat) => (
						...
						>
							...
							<div>
								...
								<p className="mt-0.5 text-sm text-[#64748B]">{cat.desc}</p>
							</div>
						</Link>
					))}
				</div>
				...
				<div className="space-y-4">
					{faqs.map((faq) => (
						...
						>
							<summary className="flex cursor-pointer items-center justify-between px-5 py-4 text-sm font-semibold text-[#0F172A] [&::-webkit-details-marker]:hidden">
								{faq.question}
								<Search className="h-4 w-4 shrink-0 text-[#94A3B8] transition-transform group-open:rotate-45" />
							</summary>
							<div className="border-t border-[#E2E8F0] px-5 py-4 text-sm leading-relaxed text-[#475569]">
								{faq.answer}
							</div>
						</details>
					))}
				...
				<div className="mt-12 rounded-xl border border-[#DBEAFE] bg-[#EFF6FF] p-6 text-center">
					...
					<Link
						href="/contact"
						className="mt-4 inline-flex items-center gap-2 rounded-xl bg-[#1E40AF] px-6 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#1E3A8A]"
					>
