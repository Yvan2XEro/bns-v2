 ...
 * aliases, its attribute names and the values those attributes offer. That
 * corpus is ~500 option values and 31 alias lists, which covers the common ads
 * well and says nothing at all about the rest.
 *
 * Saying nothing is the point. A wrong category that the seller does not
 * notice buries their ad where no buyer looks, so a guess is only returned
 * when it is both strong enough on its own and clearly ahead of the runner-up
 * (see `MIN_SCORE` and `MARGIN`). Below either bar the caller gets `null` and
 * asks the question instead of answering it badly.
 *
 * ── Keep in sync ──────────────────────────────────────────────────────────
 * The block between the SHARED CORE markers is duplicated verbatim in
 * `packages/web/src/lib/category-suggest.ts`, and `categorySuggest.test.ts`
 * fails if the two drift. Web and mobile scoring the same title differently
 * would be a bug no one would think to look for.
 */
